 ...
 * @author Piter
 */
public class M_dodajPostController implements Initializable {
    ...
    private void wczytajID() {
        ...
        try {
            String zapytanie = "SELECT max(numer) as id FROM post_ochrony where sektor_symbol=?";
    ...
    private void wczytajDane() {
        try {
            String zapytanie = "SELECT * FROM post_ochrony where numer=? and sektor_symbol=?";
            PreparedStatement st = BazyProjekt.polaczenie.doPrepareStatement(zapytanie);
            st.setInt(1, id);
    ...
    private void handleOK(ActionEvent event) throws IOException {
        ...
        if (event.getSource() == bOK) {
            try {
                ...
                BazyProjekt.polaczenie.doZapytanie("LOCK TABLE post_ochrony IN EXCLUSIVE MODE");
                wczytajID();
                zapytanie = "INSERT INTO post_ochrony(numer,sektor_symbol)\n"
                        + "VALUES(?,?)";
